...
describe('DiscoverPage', () => {
  ...
  // Regression, LOS-230: this used to fetch the library and keep the reading
  // rows, which only ever saw the 24 most recently added — a book started a
  // while ago disappeared from the section entirely. The filter has to be the
  // server's, so the page size stops deciding what is shown.
  it('asks the server for reading entries rather than filtering a page here', async () => {
    ...
    expect(mockedSearchLibrary).toHaveBeenCalledWith({ status: 'reading' });
  });
  ...
  it('navigates to Search with the query when an example pill is clicked', async () => {
    mockedSearchLibrary.mockResolvedValue({ entries: [], total: 0, page: 1, pageSize: 24, query: '' });
  ...
  it('stays quiet (no error banner) when the call 401s because the visitor is logged out', async () => {
    mockedSearchLibrary.mockRejectedValue(new ApiError(401, 'Authentication required'));
